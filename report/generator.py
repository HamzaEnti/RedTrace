...
class _ReportBase(ReportGenerator):
    ...
    Tots dos formats (JSON i text) comparteixen exactament la mateixa
    lògica de transformació de dades; l'única diferència és com es
    serialitzen al disc. Per evitar duplicar codi, centralitzem aquí
    l'extracció i l'emmagatzemem en dues estructures de dades:
      - self._events  → deque (cua FIFO) d'esdeveniments per node + fases
      - self._meta    → dict pla amb les mètriques globals de la ruta
    """
    ...
    def _populate(self, report: AnalysisReport) -> None:
        """Extreu totes les dades de l'objecte AnalysisReport i les
        emmagatzema en self._meta i self._events.

        Aquesta funció es crida sempre abans de serialitzar, de manera
        que els dos generadors (JSON i text) tinguen les mateixes dades.
        """
...
class JSONReport(_ReportBase):
    """Serialitza l'informe com a fitxer JSON estructurat.

    El fitxer resultant conté tres claus de primer nivell:
      - metadata:            mètriques globals de la ruta
      - events:              llista ordenada d'events (nodes + fases IDS/cicles)
      - node_classifications: diccionari {node_id → nivell de risc} per a tot el graf
    """
...
class TextReport(_ReportBase):
    """Genera un informe llegible per humans en format text pla (.txt).

    La sortida inclou les mateixes dades que el JSON però formatejades
    com un informe clàssic: capçalera, ruta pas a pas, cicles i IDS.
    """
